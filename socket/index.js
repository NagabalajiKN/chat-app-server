...
const initSocket = (server, corsOptions) => {
  ...
  io.on('connection', (socket) => {
    socket.on('USER_ONLINE', (userId, socketId) => {
      ...
      const prevSocketId = userExists?.socketId || null;
      if (userExists && prevSocketId !== socketId) {
        onlineUsers = onlineUsers.map((user) =>
          user.userId === userId ? { ...user, socketId: socketId } : user
        );
      } else if (!userExists) {
        onlineUsers.push({
          userId,
    ...
    socket.on('SEND_MESSAGE', async (messageData) => {
      ...
      const messageReader = await Message.find()
        .all('users', filter)
        .select(['readers'])
        .sort({ createdAt: -1 })
        .lean();
      ...
      const clientId =
        type === 'user'
          ? onlineUsers.find(({ userId }) => userId === receiver)?.socketId
          : receiver;
      if (clientId) {
    ...
    socket.on('ROOM_CREATED', ({ name, creator, invitedUser }) => {
      invitedUser.forEach((invitedUser) => {
        ...
        if (socketId) {
          socket.to(socketId).emit('INVITED_TO_ROOM', { message: `${creator} has added you to the ${name} chat room` });
        }
      });
    });
  });
