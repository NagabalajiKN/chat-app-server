...
const getUnreadCount = asyncHandler(async (type, from, to) => {
  ...
  const messageReaders = await Message
    .find({ sender: { $ne: from } }) // Exclude messages sent by the user
    .all('users', filter)
    .select(['readers'])
    .sort({ createdAt: -1 })
    .lean();
  
  // Filter messages where the user's ID is not in readers
...
const updateMessageReadStatus = asyncHandler(async (req, res) => {
  try {
    // Mark messages in chatId as read by userId
    const { userId } = req.params;
    const { type, chatId } = req.query;
    ...
    // Find all messages in the chat where sender is not the user
    const messages = await Message
      .find({ sender: { $ne: userId }})
      .all('users', filter)
      .sort({ createdAt: 1 });

    // Get a map of message IDs to readers
    const messageReaderMap = messages.reduce((prev, curr) => {
      return {...prev, [curr._id.toHexString()]: curr.readers };
    }, {});

    // Check if userId is not in the readers for each message, add if not
    Object.entries(messageReaderMap).forEach(([key, value]) => {
      const userHasRead = value.indexOf(userId) > -1;
      if (!userHasRead) messageReaderMap[key].push(userId); // Add if not already read
    });
